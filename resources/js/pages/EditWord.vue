...
const fetchWord = async () => {
    ...
    try {
        const { data } = await axios.get(`/api/words/${props.wordId}`);
        const fetched: Word | undefined = data?.data;
        if (fetched) {
            word.value = fetched;
        } else {
        ...
        errorMessage.value = error?.response?.data?.message ?? 'Unable to load word.';
    } finally {
        isLoading.value = false;
    }
};

const handleUpdated = (updated: Word) => {
    word.value = updated;
...
<template>
    ...
    <main class="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-100 via-white to-white px-4 py-16 text-slate-800">
        ...
        <section class="mx-auto flex w-full max-w-4xl flex-col gap-8">
            ...
            <div v-if="isLoading" class="rounded-2xl border border-dashed border-slate-200 p-10 text-center text-sm text-slate-500">
                Loading word...
            </div>
            <WordForm v-else-if="word" mode="edit" :initial-word="ord" @word-updated="handleUpdated" @cancel="handleCancel" />
            <div v-else class="rounded-2xl border border-dashed border-slate-200 p-10 text-center text-sm text-slate-500">Word not found.</div>
        </section>
    </main>
</template>
